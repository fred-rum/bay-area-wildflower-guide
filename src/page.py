...
class Page:
    ...
    def resolve_group(self, rank):
        ...
        # group = None means no group has been set yet.
        # This is different than self.group[rank] = None, which means that
        # the group is conflicting.  If we find a conflict, we immediately
        # set self.group[rank] to None and exit, so the value of group
        # won't matter.
        group = None

        # Resolve the group for all children.
        for child in self.child:
            child.resolve_group(rank)

        # If at least one descendent specifies a group and no descendent
        # disagrees with that group, we set self.group[rank] later with a
        # call to set_group().
        # If the group is found to be conflicting, we set self.group[rank]
        # to None and immediately exit.
        for child in self.child:
    ...
    def find_property(self, prop, searched_set=None):
        ...
        if prop in self.prop:
            ...
            for rank in self.prop[prop]:
                ...
                #
                # x['top'] indicates the highest ancestor that is
                #   - in the existing hierarchy
                #   - no higher than x['match']
                # x['top'] is initially set the same value of x['match'], but
        ...
        for rank in self.group:
            if self.group[rank] and rank in result:
                x = result[rank]
                if not x['match']:
