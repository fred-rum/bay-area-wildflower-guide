...
with open(root_path + '/data/observations.csv', mode='r', newline='', encoding='utf-8') as f:
    ...
    subphylum_idx = header_row.index('taxon_subphylum_name')
    ...
    for row in csv_reader:
        ...
        if page:
            ...
            subphylum = row[subphylum_idx]
            if family and subphylum == 'Angiospermae':
                page.set_group('family', family)
...
for family, child_set in group_child_set['family'].items():
    ...
    if com == 'n/a':
        page = find_page1(family)
    else:
        page = find_page2(com, family)
